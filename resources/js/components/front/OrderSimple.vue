                        ...
                        <div v-if="plan.in_stock > 0" class="card card-plan mb-3">
                            ...
                            <div class="card-plan-buy">
                                <div class="input-group text-center">
                                    <button @click="changeQuantityMinus(plan)" class="btn btn-primary btn-secondary">-</button>
                                    <input v-model="selected.plans.find(p => p.name == plan.name).quantity" type="number" min="0" :max="plan.in_stock" disabled class="form-control fw-bold text-center">
                                    <button @click="changeQuantityPlus(plan)" class="btn btn-primary">+</button>
                                </div>
                            </div>
                        </div>
                    </template>
...
export default {
    ...
    methods: {
        ...
        changeQuantityPlus(plan) {
            ...
            if(selectedPlan.quantity < plan.in_stock) {
                selectedPlan.quantity = selectedPlan.quantity + 1
            } else {
